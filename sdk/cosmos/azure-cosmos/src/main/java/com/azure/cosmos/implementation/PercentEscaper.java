...
public final class PercentEscaper {
    ...
    static {
        ...
        SAFE_CHARACTERS = new boolean[128];
     ...
     */
    public PercentEscaper(String safeCharacters) {
        this.safeCharacterPoints = Arrays.copyOf(SAFE_CHARACTERS, 128); // 128 works as only ASCII characters are safe.
        if (Strings.isNotEmpty(safeCharacters)) {
    ...
    public String escape(String original) {
        ...
        while (index < end) {
            ...
            if (codePoint < 128 && safeCharacterPoints[codePoint]) {
    ...
    private static int getCodePoint(String original, int index, int end) {
        ...
        if (!Character.isSurrogate(char1)) {
            ...
            if (index == end) {
                throw new IllegalStateException("String contains trailing high surrogate without paired low surrogate.");
            }
